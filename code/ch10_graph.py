...
class Graph:
    ...
    def _get_vertex(self, key):
        ...
        return self._adj[key]  # return a vertex, not key itself
    ...
    def __repr__(self):
        ...
        adj_lst_repr = (
            f"{repr(v)}: {edges_repr(v.outgoing_edges())}" for v in self._adj.values()
        )
...
if __name__ == "__main__":
    # Inserting nodes and edges to build a graph
    graph1 = Graph()
    for key in range(0, 5):
        graph1.insert_vertex(key)
    ...
    graph2 = Graph()
    graph2.read_file(file_path)
    print(graph2)
    print(repr(graph2))
    print("Vertices:", graph2.get_vertices())
    print("Edges:", graph2.get_edges())
    print("Vertex count:", graph2.vertex_count())
    print("Edge count:", graph2.edge_count())
    graph2.delete_vertex(1)
    print("After deleting vertex 1:")
    print(graph2)
